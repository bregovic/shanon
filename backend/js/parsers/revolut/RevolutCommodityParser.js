// /broker/js/parsers/revolut/RevolutCommodityParser.js
...
export default class RevolutCommodityParser extends BaseParser {
  async parse(content) {
    ...
    throw new Error('Neplatný formát obsahu pro Revolut Commodity');
  }
  ...
  parsePdf(text) {
    ...
    while ((m = blockRe.exec(t)) !== null) {
      ...
      if (buyM) {
        ...
        const feeAsset = this.parseNumber(block.match(/Poplatek:\s*([0-9][0-9.,\s]*)\s*(XAU|XAG|XPT|XPD)/i)?.[1]) || 0;
        const qtyGross = (qtyNet || 0) + (feeAsset || 0);

        // najdi částku v původní fiat měně (poslední výskyt €/$/kód měny)
        let amountCur = null, currency = null;
      ...
      if (/buy|nákup|nakup/i.test(typeRaw)) {
        out.push({
          ...
          __tmp: (feeFiat != null && !Number.isNaN(feeFiat))
            ...
            : (feeAsset ? { feeAsset, feeFiat: unit != null ? unit * feeAsset : 0, feeFiatCurrency: currency } : null),
          notes: `CSV: ${typeRaw}`
        });
