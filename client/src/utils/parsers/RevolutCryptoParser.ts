...
export default class RevolutCryptoParser extends BaseParser {
    async parse(content: any): Promise<Transaction[]> {
        ...
        // Fallback or explicit CSV parsing if content is string but CSV-like logic handled elsewhere
    ...
    parsePdf(text: string): Transaction[] {
        ...
        while ((m = blockRe.exec(t)) !== null) {
            ...
            if (!rw) {
                ...
                if (alt) rw = [alt[0], alt[2], alt[1]]; // => [text, symbol, qty]
            }
            if (rw) {
    ...
    parseTabularSection(t: string): Transaction[] {
        const out: Transaction[] = [];
        const symToCur = (s: string) => (s === '€' ? 'EUR' : s === '$' ? 'USD' : (s || '').toUpperCase());
        ...
        while ((m2 = rowRe.exec(t)) !== null) {
            ...
            if (isRev) {
                out.push({
                    date: dateIso, id: symbolRaw, amount: qty, price: null,
                    // @ts-ignore
                    ex_rate: null,
                    amount_cur: 0, currency: totalCur || priceCur || 'CZK',
                    // @ts-ignore
                    amount_czk: 0,
